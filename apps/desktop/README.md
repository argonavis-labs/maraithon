...
Browser work uses installed Chrome on that Mac, following `~/bliss/runner`'s local browser pattern. Action cards carry a Runner-badged Chrome mark. The native relay keeps a persistent profile per account and a tab per task. See [Runner local Chrome](../../docs/runner-local-chrome.md) for branding, profile behaviour, and verification.
