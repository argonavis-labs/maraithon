...
/// Persists the (guid → modified_at) map of every reminder we've
/// pushed to the server, so a restart only re-uploads reminders whose
/// EventKit `lastModifiedDate` has advanced.
///
/// Reminders differ from notes/voice-memos: there's no monotonic
/// `Z_PK` we can resume from. EventKit gives us per-row
/// `calendarItemIdentifier` (string) plus `lastModifiedDate`, and a
/// reminder's `lastModifiedDate` advances every time the user toggles
/// completion, edits the title, etc. The cursor stores the last
/// modification timestamp we've shipped for each guid; the source
/// re-pushes any reminder whose current `lastModifiedDate` is greater.
///
/// Stored in `UserDefaults` under
/// `com.maraithon.companion.reminders.cursor` as a `[String: Double]`
/// (guid → seconds since the reference date). Tests can substitute
/// their own suite via the initializer.
struct RemindersCursor: @unchecked Sendable {
    static let defaultsKey = "com.maraithon.companion.reminders.cursor"
    ...
    /// Read the persisted (guid → timestamp) snapshot. Returns an
    /// empty map on first run or when the value is corrupt — we
    /// re-sync everything in that case, and the server's idempotent
    ...
    /// `RemindersSource.clearLocalState`.
    func reset() {
