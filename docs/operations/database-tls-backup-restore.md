...
This runbook defines the production PostgreSQL role, connection, migration,
backup, PITR, and restore boundaries for the durable runtime. Keep database
URLs, passwords, Fly tokens, key material, backup manifests, and provider
credentials outside the repository and command output.
...
| --- | --- | --- |
| `DATABASE_URL` | `maraithon_runtime` | Application and ordinary runtime storage operations |
| `MARAITHON_MIGRATOR_DATABASE_URL` | `maraithon_migrator` | DDL and `maraithon.coordination.backfill` |
...
| `MARAITHON_INCIDENT_DATABASE_URL` | `maraithon_incident_operator` | Termination and binding/key lifecycle evidence |
| `VAULT_ROTATION_DATABASE_URL` | `maraithon_incident_operator` | Vault ciphertext rotation/retirement wrapper |

`VAULT_ROTATION_DATABASE_URL` is a separately named binding to the incident
role, not a seventh role. Operator tasks load production configuration, replace
only the Repo URL, verify the expected username is distinct from `DATABASE_URL`,
and rebuild transport options for that URL.
...
Snapshot format finalization and the initial privacy-retention constraint
finalizer also perform reviewed DDL. Run their complete rollout sequence with a
process-scoped `DATABASE_URL="$MARAITHON_MIGRATOR_DATABASE_URL"`, then discard
that process environment. Ongoing snapshot writes, retention cycles, and
privacy requests return to the runtime role.
...
every recoverable historical path have moved past it. The mandatory order is:

1. take the fixed registry locks, prove the global live old-tag count zero, and
   persist its PostgreSQL-clock proof;
...
3. complete an isolated restore recovered through a point strictly newer than
   the proof and attest the content-free evidence with an expiry;
4. immediately retake all current source locks and obtain a fresh global zero
   recount through the retirement preflight;
5. only then remove that domain's old read key.

An old proof row, cached count, expired attestation, pre-proof backup, or backup
without a successful restore is not authority. See [Durable payload
lifecycle](durable-payload-lifecycle.md) for the Vault and binding wrappers and
evidence tuple.
