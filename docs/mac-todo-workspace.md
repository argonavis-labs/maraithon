...
The header keeps the title, one meta line, and two controls (Details, Done). Below
it, a work column leads with Maraithon's read and shows at most two action cards
under "Next action": a live review opens (editable draft, Send, Cancel), a
suggestion from the brief shows as a card with Prepare, and everything else is
either one compact row or not a card at all. When nothing specific is ready,
one card offers to prepare the todo. People appear as chips that ask Maraithon
about that person. The chat pane sits beside the work column (stacked below it
in narrow windows) with the composer pinned at the bottom: Return sends,
Shift-Return adds a line, a message sent mid-run queues until the run settles.
Turns follow the runner-next session transcript: the user's words in a quiet
bubble, Maraithon's reply as chromeless prose with its tool work folded into one
line and a working indicator driven by run status. Details (status, timing,
notes, source context, dismiss) live in a sheet. The design is recorded in
`docs/superpowers/specs/2026-09-13-todo-workspace-focus-design.md`. Each todo
retains its conversation, unsent composer text, and edited drafts while
navigating between todos in the same Todos view.
...
Consent does not send a message; messages remain separate reviews. Calendar
blocks are different: from Sep 13 2026, `calendar_create_event`,
`calendar_update_event`, and `calendar_cancel_event` run as soon as they are
prepared (`Toolbox` `@direct_execution_action_types`, through the same
`confirm_and_execute` path a tapped card uses), so "book it" books it and the
card arrives already marked "Saved to calendar". Maraithon still only touches
events it created.

The model behind every call made for a user is one setting: `users.assistant_model`,
edited on the web Settings page ("Assistant model"). `Maraithon.LLM.UserModel`
binds the acting user at run, agent, and background-job boundaries and pins the
chosen model at the provider choke point. Blank means the workspace default.
