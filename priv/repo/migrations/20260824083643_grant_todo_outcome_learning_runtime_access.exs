defmodule Maraithon.Repo.Migrations.GrantTodoOutcomeLearningRuntimeAccess do
  use Ecto.Migration

  def up do
    execute_compatible("""
    DO $todo_outcome_learning_grants$
    BEGIN
      REVOKE ALL ON TABLE public.todo_learning_events FROM PUBLIC;
      GRANT ALL ON TABLE public.todo_learning_events TO maraithon_migrator;
      GRANT SELECT, INSERT, UPDATE, DELETE
        ON TABLE public.todo_learning_events TO maraithon_runtime;
      ALTER TABLE public.todo_learning_events OWNER TO maraithon_object_owner;
    END
    $todo_outcome_learning_grants$;
    """)
    ...
    BEGIN
      ...
      IF live_functions IS DISTINCT FROM stored_functions OR
        ...
        RAISE EXCEPTION 'Todo outcome-learning privacy manifest refresh found unrelated catalog drift'
      ...
      IF EXISTS (
        ...
        RAISE EXCEPTION 'Todo outcome-learning privacy manifest refresh found an unknown column ACL grantee'
          USING ERRCODE = 'check_violation';
      END IF;
      ...
      IF public.privacy_protocol_catalog_ready() IS NOT TRUE THEN
        RAISE EXCEPTION 'Todo outcome-learning privacy manifest refresh did not restore readiness'
          USING ERRCODE = 'check_violation';
      END IF;
