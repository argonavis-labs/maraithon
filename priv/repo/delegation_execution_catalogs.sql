-- Frozen reviewed catalog update for migration 20260915134306.
DO $catalogs$
DECLARE prior record; stored_functions jsonb; stored_triggers jsonb; stored_catalogs jsonb;
  live_functions jsonb; live_triggers jsonb; live_catalogs jsonb;
  payload jsonb; reviewed jsonb; item record; section text;
  tables constant text[] := ARRAY['telegram_prepared_actions','telegram_assistant_runs'];
BEGIN
  SELECT * INTO STRICT prior FROM delegation_execution_prior_catalogs;
  stored_functions := prior.functions;
  stored_triggers := prior.triggers || jsonb_build_object('public.telegram_prepared_actions.guard_delegated_prepared_action', NULL);
  ...
  IF (live_functions - ARRAY['privacy_protocol_catalog_ready()', 'guard_delegation_record()'])
       IS DISTINCT FROM (prior.functions - ARRAY['privacy_protocol_catalog_ready()', 'guard_delegation_record()']) OR
     EXISTS (SELECT 1 FROM jsonb_each(prior.triggers) p WHERE live_triggers->p.key IS DISTINCT FROM p.value) OR
     (live_catalogs - tables) IS DISTINCT FROM (prior.catalogs - tables) THEN
    RAISE EXCEPTION 'Delegated execution found unrelated privacy catalog drift';
  END IF;
  ...
  UPDATE public.privacy_protocol_manifests SET
    function_fingerprints = live_functions, trigger_fingerprints = live_triggers, catalog_fingerprints = live_catalogs,
  ...
  FOREACH section IN ARRAY ARRAY['functions','catalogs','constraints','indexes','triggers'] LOOP
    FOR item IN SELECT * FROM jsonb_each(payload->section) LOOP
      IF (section = 'functions' AND item.key IN ('guard_delegation_record()', 'durable_payload_digest_part(text,jsonb,text)')) OR
         (section = 'catalogs' AND item.key = ANY(tables)) OR
         (section = 'constraints' AND item.key IN ('telegram_prepared_actions.prepared_action_delegation_shape', 'telegram_assistant_runs.assistant_run_delegation_shape')) OR
         (section = 'indexes' AND item.key IN ('telegram_prepared_actions.prepared_action_delegation_turn_unique', 'telegram_prepared_actions.telegram_prepared_actions_delegation_id_index')) OR
         (section = 'triggers' AND item.key = 'telegram_prepared_actions.guard_delegated_prepared_action') THEN
        reviewed := jsonb_set(reviewed, ARRAY[section, item.key], item.value, true);
      END IF;
    END LOOP;
  END LOOP;
  IF reviewed IS DISTINCT FROM payload THEN
    RAISE EXCEPTION 'Delegated execution found unrelated durable catalog drift';
  END IF;
  ...
  UPDATE public.durable_payload_protocol_manifests SET
    catalog_manifest = reviewed, manifest_digest = public.digest(convert_to(reviewed::text, 'UTF8'), 'sha256'),
    updated_at = timezone('UTC', clock_timestamp())
  ...
  IF NOT public.privacy_protocol_catalog_ready() OR NOT public.durable_payload_catalog_ready() OR
    ...
    RAISE EXCEPTION 'Delegated execution did not preserve protocol readiness';
  END IF;
END $catalogs$;
