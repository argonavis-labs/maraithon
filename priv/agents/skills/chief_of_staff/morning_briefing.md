...
- Use the `todos` array as the durable open-work creation surface for any follow-up, relationship-linked task, prep task, personal/family logistic, or owner/status check that should survive after the Telegram message. The runtime sends each work item after the briefing as an individual Telegram card with Done, Dismiss, Important, and Not Important actions, so every item must be worth a separate decision.
