...
- Deploys run in CI, not from a developer machine: push to `main` triggers `.github/workflows/deploy-gcp.yml`, which authenticates as `admin-deployer@maraithon.iam.gserviceaccount.com` through workload identity and runs the fast path. To ship server changes, commit and push; do not run `make deploy` locally or create service-account keys. CI reads the Agent termination public key from the repo variable `AGENT_TERMINATION_ATTESTATION_PUBLIC_KEY`.
...
- The same proofs fingerprint the shape of 47 catalogued tables (`users`, `todos`, `agents`, the telegram and runtime tables, and more). A migration that adds, drops, or retypes a column on one of them must also refresh `privacy_protocol_manifests` (and `durable_payload_protocol_manifests` when the table is a durable payload catalog) in the same migration, following the reviewed-refresh pattern in `20260908193000_add_owned_todo_workflow.exs` and `20260913223000_register_assistant_model_in_privacy_manifest.exs`. Otherwise `privacy_protocol_catalog_ready()` turns false and the key fence boot guard refuses the new revision.
