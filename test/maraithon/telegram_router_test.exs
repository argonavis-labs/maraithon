defmodule Maraithon.TelegramRouterTest do
  ...
  test "medium-confidence rules ask for confirmation and a plain-text yes confirms them", %{
    ...
    assert confirmation_prompt.text =~ "Remember this for future triage?"
    assert confirmation_prompt.text =~ "Treat investors as urgent"
    assert confirmation_prompt.text =~ "Reply <code>yes</code> to remember it"
  ...
  test "draft generation prompt includes long-term memory summaries and style rules", %{
    ...
    assert prompt =~ "Write a concise email reply in the operator's first-person voice"
    assert prompt =~ "Operator voice guidance JSON"
  ...
  test "model-unavailable Telegram fallback avoids dead-end retry copy" do
    ...
    assert reply.text =~ "checked answer"
    assert reply.text =~ "did not guess"
    refute reply.text =~ "I "
    refute String.contains?(String.downcase(reply.text), "source-backed")
