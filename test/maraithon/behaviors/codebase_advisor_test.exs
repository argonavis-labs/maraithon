defmodule Maraithon.Behaviors.CodebaseAdvisorTest do
  ...
  describe "handle_wakeup/2" do
    test "requests LLM review for pending file" do
      ...
      assert prompt =~ "senior engineering advisor"
      assert prompt =~ "Return one of these forms"
      assert prompt =~ "## Findings"
      assert prompt =~ "Evidence:"
      assert prompt =~ "Fix:"
      assert prompt =~ "Verify:"
      assert prompt =~ "## Insufficient Context"
  ...
  describe "handle_effect_result/3" do
    test "processes LLM review response" do
      ...
      report = File.read!(state.output_path)
      assert report =~ "# Engineering Review Recommendations"
      assert report =~ "Generated by Maraithon engineering review"
      refute report =~ "CodebaseAdvisor"
    end
