defmodule Maraithon.Behaviors.RepoPlannerTest do
  ...
  describe "handle_wakeup/2 - planning phase" do
    test "analyzing phase requests LLM analysis" do
      ...
      assert prompt =~ "senior repository planner"
      assert prompt =~ "Return ONLY valid JSON"
      assert prompt =~ "\"missing_context\""
      assert prompt =~ "Do not guess paths"
    end
    ...
    test "generating phase requests plan generation" do
      ...
      prompt = params["messages"] |> List.first() |> Map.fetch!("content")
      assert prompt =~ "implementation-ready plan"
      assert prompt =~ "## Objective"
      assert prompt =~ "## Files to Change"
      assert prompt =~ "## Verification"
      assert prompt =~ "## Rollout / Risk"
      assert prompt =~ "## Non-goals"
      assert prompt =~ "Do not expose internal automation labels"
    end
  end
  ...
  describe "handle_effect_result/3 - planning LLM results" do
    ...
    test "writes plan files as operator-ready markdown without runtime ids" do
      output_path = Path.join(@test_dir, "plans")
      ...
      response = %{content: "## Objective\nShip the smallest useful slice."}

      {:emit, {:plan_generated, payload}, _new_state} =
        RepoPlanner.handle_effect_result({:llm_call, response}, state, @context)

      assert payload.plan_file_path
      report = File.read!(payload.plan_file_path)
      assert report =~ "# Implementation Plan"
      assert report =~ "**Task:** Add feature"
      assert report =~ "**Generated by:** Maraithon repository planning"
      assert report =~ "## Objective"
      refute report =~ "Task ID"
      refute report =~ "test-id"
    end
  end
