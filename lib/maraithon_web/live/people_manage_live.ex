defmodule MaraithonWeb.PeopleManageLive do
  ...
  def mount(_params, _session, socket) do
    {:ok,
     assign(socket,
       page_title: "People",
       current_path: "/operator/people/manage",
  ...
  def handle_event("clear_filters", _params, socket) do
    {:noreply, push_patch(socket, to: ~p"/operator/people/manage")}
  end
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_path={@current_path} current_user={@current_user}>
      <div class="space-y-6">
        ...
        >
          <:actions><.button navigate={~p"/operator/people"} variant="plain">Back to network</.button></:actions>
        </.page_header>
  ...
  defp people_path(filters, extra_params \\ %{}) do
    ...
    if map_size(query) == 0,
      do: ~p"/operator/people/manage",
      else: ~p"/operator/people/manage?#{query}"
  end
  ...
  defp current_path_from_uri(uri) do
    ...
    |> Map.get(:path)
    |> case do
      nil -> "/operator/people/manage"
      "" -> "/operator/people/manage"
      path -> path
    end
  end
