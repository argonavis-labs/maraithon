defmodule Maraithon.Runtime.GmailSourceReplay do
  @moduledoc """
  Validates and identifies a bounded, account-local Gmail source replay.

  Replays use dedicated discovery and closure cursor chains. They never read
  or write the live Gmail watermarks.
  """
  ...
  def build(_account, _lower, _upper, _now), do: {:error, :invalid_gmail_source_replay}
    ...
    case Map.get(payload, "source_replay_mode") do
      ...
      "historical" ->
        with {:ok, replay} <-
          ...
          _invalid -> {:error, :invalid_gmail_source_replay_payload}
        end

      _invalid ->
        {:error, :invalid_gmail_source_replay_payload}
    end
  end

  def from_payload(_account, _payload, _role, _now),
    do: {:error, :invalid_gmail_source_replay_payload}
  ...
  @doc "Verifies both exact proof chains for a completed Gmail replay."
  def verify(account_id, lower, upper) when is_integer(account_id) do
    with %ConnectedAccount{} = account <- Repo.get(ConnectedAccount, account_id),
      ...
      false -> {:error, :gmail_source_replay_role_manifests_differ}
      {:error, _reason} = error -> error
      _invalid -> {:error, :gmail_source_replay_verification_failed}
    end
  end

  def verify(_account_id, _lower, _upper),
  ...
  def verify_role(%ConnectedAccount{} = account, replay, role)
    ...
    with {:ok, cycle} <- replay_cycle(account.id, kind, role),
      ...
      version when is_integer(version) -> {:error, :gmail_source_replay_proof_version_mismatch}
      {:error, _reason} = error -> error
      _invalid -> {:error, :gmail_source_replay_verification_failed}
    end
  end

  def verify_role(_account, _replay, _role),
    do: {:error, :invalid_gmail_source_replay}

  defp gmail_account?(%ConnectedAccount{provider: provider}) when is_binary(provider) do
    provider == "google" or String.starts_with?(provider, "google:")
  end

  defp gmail_account?(_account), do: false

  defp replay_for_bounds(%ConnectedAccount{} = account, lower, upper) do
    with true <- gmail_account?(account),
  ...
  defp replay_cycle(account_id, kind, role) do
    ...
    case cycles do
      [cycle] -> {:ok, cycle}
      [] -> {:error, {:gmail_source_replay_cycle_missing, role}}
      _many -> {:error, {:gmail_source_replay_cycle_not_unique, role}}
    end
  end
  ...
  defp verify_cycle_bounds(cycle, replay) do
    if cycle.lower_cursor == Integer.to_string(replay.lower) and
      ...
      {:error, :gmail_source_replay_cycle_bounds_mismatch}
    end
  end
  ...
  defp verified_items(cycle, replay) do
    ...
    if complete?, do: {:ok, items}, else: {:error, :gmail_source_replay_item_bounds_mismatch}
  end
  ...
  defp verify_cursor(account_id, kind, upper) do
    ...
    case SourceCursors.get(account_id, kind) do
      ...
      _other -> {:error, :gmail_source_replay_cursor_incomplete}
    end
  end
  ...
  defp replay_reference(account_id, lower, upper) do
    ["gmail-source-replay-v1", account_id, lower, upper]
    |> Enum.join(":")
    |> then(&:crypto.hash(:sha256, &1))
    |> Base.url_encode64(padding: false)
  end

  defp watermark_kind(role, reference), do: "gmail_#{role}_replay:#{reference}"
