defmodule Maraithon.ReviewerSeed do
  @moduledoc """
  Seeds the App Store review demo account with a small, representative,
  idempotent dataset so Apple's reviewer lands in a populated account
  (Today / Work / People / Chat) instead of an empty shell.

  Invoked by the `mix maraithon.seed_reviewer` task in dev, or directly via
  `Maraithon.ReviewerSeed.seed/0` on a running prod node (the release has no
  Mix). Run once after deploy — NOT on every boot.
  ...
  @doc """
  Seed the reviewer demo account. Resolves the reviewer email from
  `APP_REVIEW_BYPASS_EMAIL` (falling back to `reviewer@maraithon.com`).

  Returns `{:ok, user_id}` or `{:error, reason}`.
  """
  def seed(email \\ reviewer_email()) do
    case Accounts.get_or_create_user_by_email(email) do
      {:ok, user} ->
        ...
        Logger.info("Seeded reviewer demo account for #{user.id}")
        {:ok, user.id}
