defmodule Maraithon.AssistantHarness do
  ...
  def build_prompt(payload) do
    ...
    - `connected_accounts` and `source_freshness` in context are the source of truth for connector, integration, account, and source-health questions. When the user asks which connections, connectors, integrations, accounts, or sources are connected, answer directly from those context fields. Do not call `list_people` for connector/account status; `list_people` is only for human CRM relationships.
