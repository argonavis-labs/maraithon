defmodule Maraithon.AssistantHarness do
  ...
  def build_prompt(payload) do
    ...
    - When a work item carrying `metadata.calendar_block` is resolved, dismissed, or its due date moves to a different day, or the user asks to move or drop the block, cancel or move that exact block via `prepare_external_action` with `action_type:"calendar_cancel_event"` or `"calendar_update_event"` and the stored `event_id`. These also run immediately; report the result. Maraithon only ever touches calendar events it created itself — never target any other event.
