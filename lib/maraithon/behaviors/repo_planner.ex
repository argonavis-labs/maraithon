defmodule Maraithon.Behaviors.RepoPlanner do
  ...
  defp build_analysis_prompt(task, index) do
    ...
    """
    You are a senior repository planner analyzing a codebase to prepare implementation work.
    ...
    Use only files present in the supplied file tree, summaries, or file list. Do not guess paths.
    ...
    Use `missing_context` when a file, command output, API contract, or product decision is required before planning safely.
    """
  end
  ...
  defp build_plan_prompt(task, index) do
    ...
    """
    You are a senior repository planner creating an implementation-ready plan.
    ...
    ## Instructions
    Create a concise, actionable implementation plan in Markdown. Use this structure:

    ## Objective
    State the smallest useful outcome in one or two sentences.

    ## Files to Change
    List existing files to modify and new files to create. Explain why each file is in scope.

    ## Implementation Steps
    Give ordered, concrete steps. Include code snippets only when they remove ambiguity.

    ## Verification
    Name the tests, commands, assertions, or manual checks that prove the work.

    ## Rollout / Risk
    Call out migration, compatibility, permissions, performance, and user-facing risks.

    ## Non-goals
    Name anything intentionally out of scope for the first launchable slice.

    ## Insufficient Context
    Include this section only when the context is too thin; name the smallest missing input.

    Do not expose internal automation labels or runtime metadata. Prefer the smallest reversible milestone over a broad rewrite.
    """
  end
  ...
  defp write_plan_file(state, task, content) do
    ...
    **Generated by:** Maraithon repository planning
