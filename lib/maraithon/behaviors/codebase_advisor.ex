defmodule Maraithon.Behaviors.CodebaseAdvisor do
  ...
  defp build_review_prompt(file_path, content, state) do
    ...
    """
    You are a senior engineering advisor preparing an engineering review brief.

    Focus areas: correctness, reliability, security, observability, performance, maintainability, and missing tests.

    Return one of these forms:

    ## Findings
    1. **Severity: high|medium|low — Category**
       - Evidence: cite the specific function, behavior, branch, or missing test visible in the supplied file.
       - Impact: explain the user-facing, operational, or engineering risk.
       - Fix: give the smallest concrete change an engineer should make.
       - Verify: name the test, assertion, or manual check that proves the fix.

    ## No Material Findings
    One sentence naming the strongest remaining risk or test gap.

    ## Insufficient Context
    One sentence naming the smallest additional file, runtime behavior, or test needed.

    Do not nitpick style or restate the file. Do not expose internal automation labels.
  ...
  defp render_markdown(state) do
    ...
    Generated by Maraithon engineering review
