defmodule Maraithon.TelegramAssistant.DeliveryPlanner do
  ...
  alias Maraithon.TelegramAssistant.{Context, ProactiveCandidate, ProactiveQueue, PushBroker}
      ...
      case candidates do
        ...
        [_ | _] ->
          with chat_id when is_binary(chat_id) <- telegram_destination(user_id, opts),
               ...
               {:ok, plan} <- AssistantHarness.plan_delivery(payload, opts) do
            planned = persist_plan(candidates, plan)
  ...
  defp build_payload(user_id, chat_id, candidates, opts) do
    %{
      user_id: user_id,
      chat_id: chat_id,
      candidates: Enum.map(candidates, &candidate_snapshot/1),
      context:
  ...
  defp candidate_snapshot(%ProactiveCandidate{} = candidate) do
    %{
      id: candidate.id,
  ...
  defp persist_plan(candidates, plan) do
    disposition_by_id =
      plan
